"""
Generates the final report as a single Markdown file, pulling in the
actual results already produced by scripts 01-08. Run this last.
"""
import pandas as pd

df = pd.read_csv('data/cleaned_weather.csv')

report = f"""# Weather Trend Forecasting — Data Science Report
...
- **Rows:** {len(df):,} daily weather records
- **Locations:** {df['location_name'].nunique()} cities across {df['country'].nunique()} countries
...
Built a daily global-average-temperature time series ({df['last_updated'].nunique() if 'last_updated' in df.columns else 'N/A'} unique days)
